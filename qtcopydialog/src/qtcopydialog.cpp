...
static QString formatSize(qint64 size)
{
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (div < base) {
            ...
            switch (i) {
            case 1:
                str = QtCopyDialog::tr("%1 kB").arg(val, 0, 'f', 1);
                break;
            case 2:
                str = QtCopyDialog::tr("%1 MB").arg(val, 0, 'f', 1);
                break;
            case 3:
                str = QtCopyDialog::tr("%1 GB").arg(val, 0, 'f', 1);
                break;
            default:
                break;
            }
...
void QtCopyDialogPrivate::error(int id, QtFileCopier::Error error, bool stopped)
{
    ...
    if (!handled) {
        ...
        case QtOtherDialog::Cancel  :
            fileCopier->cancelAll();
            fileCopier->retry();
            break;
        case QtOtherDialog::Skip    :
            fileCopier->skip();
            break;
        case QtOtherDialog::SkipAll :
            fileCopier->skipAll();
            break;
        case QtOtherDialog::Retry   :
            fileCopier->retry();
            break;
        default                     :
            fileCopier->retry();
